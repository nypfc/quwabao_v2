...
public final class BetFormValidate {
     ...
     */
    public static String validate(Integer guessMode, String guessNumber) {
        // 验证下注号码的合法性
        if (guessMode.equals(GuessModeEnum.MODE_1.getMode())) {
            if (guessNumber.length() != 1) {
                return "玩法一的下注号码长度不正确";
            }
            if (!guessNumberValidate(Integer.parseInt(guessNumber))) {
                return "玩法一的下注号码不正确";
            }
        } else if (guessMode.equals(GuessModeEnum.MODE_2.getMode())) {
            if (guessNumber.length() != 2) {
                return "玩法二的下注号码长度不正确";
            }
            ...
            if (!(guessNumberValidate(number1) && guessNumberValidate(number2))) {
                return "玩法二的下注号码不正确";
            }
        } else if (guessMode.equals(GuessModeEnum.MODE_3.getMode())) {
            if (guessNumber.length() != 1) {
                return "玩法三的下注号码长度不正确";
            }
