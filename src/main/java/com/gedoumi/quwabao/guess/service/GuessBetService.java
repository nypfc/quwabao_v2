...
public class GuessBetService {
    ...
    public void bet(BetForm betForm) {
        ...
        // 获取参数
        BigDecimal bet = new BigDecimal(betForm.getBet());
        Integer guessMode = Integer.parseInt(betForm.getGuessMode());
        String guessNumber = betForm.getGuessNumber();

        // 参数验证
        String message = BetFormValidate.validate(guessMode, guessNumber);
        if (message != null) {
            throw new BusinessException(CodeEnum.ParamError);
        }
        // 获取竞猜详情，判断状态
        ...
        if (!guessDetail.getGuessDetailStatus().equals(GuessDetailStatusEnum.BETTING.getValue())) {
            throw new BusinessException(CodeEnum.NotBetting);
        }
        ...
        guessBet.setBetMoney(bet);
        guessBet.setGuessNumber(Integer.parseInt(guessNumber));
