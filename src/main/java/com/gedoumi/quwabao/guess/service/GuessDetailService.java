...
import com.gedoumi.common.utils.GuessProbabilityUtil;
import com.gedoumi.common.utils.ListUtil;
...
import com.gedoumi.quwabao.guess.mapper.GuessDetailMapper;
...
public class GuessDetailService {
     ...
     * @return 竞猜详情
     */
    public GuessDetail getLatest() {
        return guessDetailMapper.selectLatest();
    }
     ...
     * @return 竞猜详情集合
     */
    public List<GuessDetail> getByIdIn(Collection<Long> collection) {
        return guessDetailMapper.selectByIdIn(collection);
    }
     ...
     * @return 竞猜详情列表
     */
    public List<GuessDetail> getGuessDetails(String page, String rows) {
        try {
            PageHelper.startPage(Integer.parseInt(page), Integer.parseInt(rows));
        } catch (NumberFormatException e) {
            throw new BusinessException(CodeEnum.ParamError);
        }
        PageInfo<GuessDetail> pageInfo = new PageInfo<>(guessDetailMapper.selectByStatus(GuessDetailStatusEnum.FINISHED.getValue()));
    ...
    public String getRanking(GuessDetail guessDetail, BigDecimal return_rate) {
        ...
        return String.valueOf(list.indexOf(1) + 1) +
                (list.indexOf(2) + 1) +
                (list.indexOf(3) + 1) +
                (list.indexOf(4) + 1) +
                (list.indexOf(5) + 1) +
